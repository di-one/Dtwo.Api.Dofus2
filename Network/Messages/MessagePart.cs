...
namespace Dtwo.API.Dofus2.Network.Messages
{
    public class MessagePart
    {
        ...
        public bool IsValid
        {
            get
            {
                bool b = Header.HasValue && Length.HasValue &&
                       Length == Data.Length;
        ...
        public bool Build(BigEndianReader reader, bool isClient)
        {
            ...
            //Console.WriteLine(System.Text.Encoding.Default.GetString(m_completeData));

            if (reader.BytesAvailable >= 2 && !Header.HasValue)
            {
                //var header = (short)(reader.Data[0] * 256 + reader.Data[1]); // Selection des 2 premiers octets du paquet

                Header = reader.ReadUShort();
                //Console.WriteLine("Header : " + Header.Value + "|" + header);
